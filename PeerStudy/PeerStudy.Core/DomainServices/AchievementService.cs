...
namespace PeerStudy.Core.DomainServices
{
    public class AchievementService : IAchievementService
    {
        ...
        //TODO: we should also consider the status of the courses(active/archived). Maybe add a dropdown with the courses status
        public async Task<StudentProfileModel> GetProgressByStudentIdAsync(Guid studentId)
        {
            ...
            studentProgress.CoursesProgress = (await unitOfWork.StudentCourseRepository.GetAllAsync(x => x.StudentId == studentId))
                .Select(x => new StudentCourseProgressModel
                {
                    ...
                    .Where(g => g.Count() > 0 && g.Any(s => s.Assignment.CompletedAt != null && s.Assignment.CourseUnit.CourseId == x.CourseId))
