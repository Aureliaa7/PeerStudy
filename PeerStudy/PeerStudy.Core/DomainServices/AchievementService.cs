    ...
    public class AchievementService : IAchievementService
    {
        ...
        public async Task<StudyGroupStatisticsDataModel> GetStatisticsDataByGroupAsync(Guid studyGroupId, Guid courseId)
        {
            ...
            return new StudyGroupStatisticsDataModel
            {
                ...
                UnlockedCourseUnits = GetUnlockedCourseUnitsStatistics(unlockedCourseUnits, students)
            };
        }
        ...
        private async Task<StudyGroupAssignmentsStatisticsDataModel> GetAssignmentsStatisticsAsync(Guid studyGroupId)
        {
            ...
            {
                NoTotalAssignments = assignments.Count(),
                NoCompletedOnTimeAssignments = assignments.Count(x => x.CompletedAt < x.Deadline),
                NoDoneLateAssignments = assignments.Count(x => x.CompletedAt > x.Deadline),
                NoMissingAssignments = assignments.Count(x => x.CompletedAt == null)
            };
        }
        ...
        {
            var unlockedCourseUnitsStatistics = (from cu in unlockedCourseUnits
                                                 join s in students on cu.StudentId equals s.Id
                                                 group cu by new { s.Id, s.FirstName, s.LastName } into g
                                                 select new StudentCourseUnitStatisticsDataModel
                                                 {
                                                     Id = g.Key.Id,
                                                     FullName = $"{g.Key.FirstName} {g.Key.LastName}",
                                                     UnlockedCourseUnits = g.Count()
                                                 })
                                                .ToList();
